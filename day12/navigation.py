...
def check_instr(position, orientation, action, value):
    if action == "N":
        move_north(position, value)
    elif action == "S":
        move_south(position, value)
    elif action == "E":
        move_east(position, value)
    elif action == "W":
        move_west(position, value)
    elif action == "L":
        turn_left(orientation, value)
    elif action == "R":
        turn_right(orientation, value)
    elif action == "F":
        move_forward(position, orientation, value)


def move_north(position, value):
    position[1] += value


def move_south(position, value):
    position[1] -= value


def move_east(position, value):
    position[0] += value


def move_west(position, value):
    position[0] -= value


def turn_left(orientation, value):
    if value == 90:
        orientation[0] -= 1
    elif value == 180:
        orientation[0] -= 2
...
def turn_right(orientation, value):
    if value == 90:
        orientation[0] += 1
    elif value == 180:
        orientation[0] += 2
